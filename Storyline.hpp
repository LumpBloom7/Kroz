...
namespace systemFS { // Experimental gameplay mechanic to simulate an actually file system that will be used within the game.
	...
	struct Directory { // Virtual directory for the terminal
		public:
			...
			void sortDir(); // Sorts subDirectories and files. Should have a worst time complexity of O((s^2) + (f^2)), best is O(s + f). Need to use merge sort, which is much faster, but this would do for now.
			void dir();
