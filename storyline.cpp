...
namespace systemFS { // Experimental gameplay mechanic to simulate an actually file system that will be used within the game.
	void File::changeDetails(std::string name)
	{
		fileName = name;
	}
	void File::changeDetails(int FileType)
	{
		fileType = FileType;
	}
	void File::changeDetails(bool isHidden)
	{
		hidden = isHidden;
	}
	...
	void File::changeContents(std::string fileContent)
	{
		content = fileContent;
	}
	...
	bool Directory::isHidden()
	{
		return hidden;
	}
	void Directory::changeDetails(std::string newName)
	{
		name = newName;
	}
	void Directory::changeDetails(bool isHidden)
	{
		hidden = isHidden;
	}
	...
	std::vector< std::shared_ptr<Directory> > Directory::getSubDirectories()
	{
	...
	void Directory::sortDir() // Sorts subDirectories and files. Should have a worst time complexity of O((s^2) + (f^2)), best is O(s + f). Need to use merge sort, which is much faster, but this would do for now.
	{
		bool again = false;
	...
	void Directory::dir()
	{
		...
		for (int a = 0; a < files.size(); a++) {
			std::cout << files[a]->getFileName() << std::endl;
		}
	}
}
...
namespace chapter1 {
	...
	int terminal( ) // Experimental terminal functionality
	{
		...
		while(true) {
			...
			// First get the command, the rest of the line will be arguments. If no arguments are provided, do the default option.
			// Computer > dir /s /a
			//       Command^  ^Arguments
			std::cin >> command;
